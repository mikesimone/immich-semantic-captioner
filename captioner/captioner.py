...
# ----------------------------
# Format:
#   IDENTITY_ALBUM_MAP="Lydia=Lydia;Me=Me;Meagan=Meagan"
# Left side = album keyword (case-insensitive whole-word match)
# Right side = canonical name to force into description
#
# NOTE: This is intentionally dumb + deterministic. You control the mapping.
IDENTITY_ALBUM_MAP = os.environ.get("IDENTITY_ALBUM_MAP", "Lydia=Lydia;Me=Me")

# Optional: noun replacement targets per identity (helps swap "a woman" -> Lydia)
# If an identity isn’t listed here, we still FORCE the name into the caption.
# Format: IDENTITY_NOUN_HINTS="Lydia=woman,girl,person;Me=man,guy,person;Meagan=woman,girl,person"
IDENTITY_NOUN_HINTS = os.environ.get(
    "IDENTITY_NOUN_HINTS",
    "Lydia=woman,girl,person;Me=man,guy,person",
)

# How to ensure identity appears if it doesn't already:
# - "prefix" => "Lydia: <caption>"
# - "suffix" => "<caption> | Lydia"
...
# Precompile album matchers (whole word)
...
_WATERMARK_PATTERNS = [
    ...
    r"\bmakeagif(?:\.com)?\b",
    r"\bmakeameme(?:\.org)?\b",
    r"\bimgflip\s+meme\s+maker\b",
    r"\bmeme\s+maker\b",
    r"\bmeme\s+generator\b",

    r"made\s+w(?:/|ith)\s+imgflip\s+meme\s+maker",
    r"made\s+with\s+imgflip",
    r"made\s+w(?:/|ith)\s+meme\s+maker",

    r"\bposted\s+in\s+r/[\w_]+\b",
    r"\br/[\w_]+\b",
    r"\breddit\b",
]
...
_JUNK_FULLCAPTION_REGEXES = [
    re.compile(r"^watch and share .* gifs on gfycat$", re.IGNORECASE),
]
...
# ----------------------------
# Florence-2: OCR-first, OCR-wins if any meaningful text exists
# ----------------------------
def load_florence():
...
# Tagging remains available but OFF by default (ENABLE_TAGS=1)
...
# ----------------------------
# Postgres helpers (YOUR schema: album + album_asset)
# ----------------------------
def pg_connect():
    if not PGPASSWORD:
