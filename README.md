...
seconds, plus an optional `creampie_count` when he gave a count without times, e.g. 0 for a
misfiled video) and always win: any count (queue, album move, studio routing) for those assets returns the
